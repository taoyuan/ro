...
module.exports = exports = function (cons, opts) {
    return new Client(cons, opts);
};

function Client(cons, opts) {
    this.cons = cons;
    this.opts = opts || {};
...
function _connect(client, cons) {
    ...
    var pinger,
        ...
        cb = opts.block || function (remote, channel) {
    ...
    d = dnode(function (remote, channel) {
        var res = cons || {};
        if (typeof cons === 'function') {
            res = cons.call(this, remote, channel);
    ...
    d.once('up', function (r) {
        ...
        client.remote = r;
        client.queue.forEach(function (fn) { fn(client.remote, client.channel) });
        client.queue = [];
        client.emit('up', client.remote, client.channel);
    });
        ...
        if (opts.ping && typeof remote.ping !== 'function') {
            ...
            pinger = setInterval(function () {
                ...
                var to = opts.timeout && setTimeout(function () {
                    ...
                    if (client.channel) client.channel.end();
                    socket.destroy();
                }, opts.timeout);
    ...
    alive = true;
    onend = function () {
        var isUp = Boolean(client.channel);
        client.channel = null;
        client.remote = null;
        client.socket = null;
        socket.destroy();
...
Client.prototype.close = function (cb) {
    ...
    if (self.channel) {
        self.channel.end();
        self.once('down', _close);
    } else if (self.socket) {
        self.socket.once('close', _close);
        self.socket.destroy();
    } else {
        _close();
    }
